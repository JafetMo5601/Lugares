    ...
    private fun updateMap(lugares: List<Lugar>) {
        if (mapReady) {
            lugares.forEach { lugar ->
                if (lugar.latitude?.isFinite() == true && lugar.length?.isFinite() == true) {
                    val marker = LatLng(lugar.latitude, lugar.length)
                    googleMap.addMarker(MarkerOptions().position(marker).title(lugar.name))
                }
            }
        }
    }
